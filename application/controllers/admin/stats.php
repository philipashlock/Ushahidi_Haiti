...
class Stats_Controller extends Admin_Controller
{
	...
	function reports()
	{
		...
		$parent_reports = array();
		$reports_per_parent_cat = array();
		foreach($data['category_counts'] as $category_id => $count) {
			...
			if($pid == 0) {
				$eval_id = $category_id;
				$parent_colors[$eval_id] = $cats[$category_id]['category_color'];
			}else{
				$eval_id = $pid;
			}
			if(!isset($parent_reports[$eval_id])) {
				$parent_reports[$eval_id] = $count;
			}else{
				foreach($parent_reports[$eval_id] as $arr_id => $arr_val){
					$parent_reports[$eval_id][$arr_id] += $count[$arr_id];
				}
			}
			if(!isset($reports_per_parent_cat[$eval_id])) $reports_per_parent_cat[$eval_id] = 0;
			$reports_per_parent_cat[$eval_id] += array_sum($count);
		...
		//$this->template->content->num_parent_categories = count($parent_reports);
		$this->template->content->reports_per_parent_cat = $reports_per_parent_cat;
		$this->template->content->parents_reports_chart = $reports_chart->chart('parent_reports',$parent_reports,$options,$parent_colors,350,350);
